...
expectOk("迁徙地图区域高亮插件", html.includes("assets/vendor/d3/d3.min.js") && app.includes("function renderInteractiveMigrationMap") && app.includes("function initializeMigrationMap") && app.includes("function resolveMigrationRegions") && app.includes("migration-region") && app.includes("active") && app.includes("d3.geoPath") && /\.migration-geo-map/.test(styles) && /\.migration-region\.active/.test(styles));
...
expectOk("前台 AI 临时预览不持久化", app.includes("async function renderPublicSurnameSearch") && app.includes("async function generatePublicAiPreview") && app.includes("function createTransientSurnameFromDraft") && app.includes("/api/public-ai-draft") && app.includes("AI 临时初稿") && app.includes("data.transient = true") && /function renderSurname\(name, options = \{\}\)/.test(app) && /const \{ transientData = null, allowCreate = true \} = options/.test(app) && /const data = transientData \|\| surnames\[name\] \|\| \(allowCreate \? createPendingSurname\(name\) : null\)/.test(app) && /renderSurname\(transientProfile\.char, \{ transientData: transientProfile, allowCreate: false \}\)/.test(app) && !/on\("searchBtn", "click", \(\) => \{\s*const value = resolveSurnameQuery\(byId\("surnameInput"\)\.value\);\s*renderSurname\(value\);/.test(app));
expectOk("前台拼音查询支持", app.includes("function resolveSurnameQuery") && app.includes("item.pinyin") && app.includes('item.info?.["拼音"]') && app.includes("const value = resolveSurnameQuery(raw)") && !app.includes("const value = normalizeSurnameInput(byId(\"surnameInput\").value);\n    renderSurname(value || \"陈\")"));
