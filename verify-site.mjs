...
const checks = [
  ...
  ["服务端 AI 环境变量优先", /const defaultAiEndpoint = readEnvText\("AI_ENDPOINT"\)/.test(server) && /const defaultAiApiKey = readEnvText\("AI_API_KEY"\)/.test(server) && /const configuredAiModel = readEnvText\("AI_MODEL"\)/.test(server) && /function resolveAiConfig/.test(server) && /endpoint: defaultAiEndpoint \|\| payload\.endpoint/.test(server) && /apiKey: defaultAiApiKey \|\| payload\.apiKey/.test(server) && /model: configuredAiModel \|\| payload\.model \|\| defaultAiModel/.test(server)],
  ...
  ["健康检查存储状态", /function getStorageStatus/.test(server) && /const probePath = join\(dataDir, `\.health-\$\{randomUUID\(\)\}\.tmp`\)/.test(server) && /const healthy = configStatus\.configReady && storageStatus\.storageWritable/.test(server) && /storageWritable/.test(server) && /runtimeDirConfigured/.test(server) && /configReady/.test(server) && /configError/.test(server) && /validateAiConfigValue/.test(server) && /AI_ENDPOINT 和 AI_API_KEY/.test(server) && /statSync\(dataDir\)\.isDirectory\(\)/.test(server) && /statSync\(backupDir\)\.isDirectory\(\)/.test(server)],
  ...
  ["前台运营台分区", /id="publicView"/.test(html) && /id="opsView"/.test(html) && /data-view="public"/.test(html) && /data-view="ops"/.test(html) && /function setAppView/.test(app)],
  ["前端管理令牌配置", /id="adminToken"/.test(html) && /function getAdminHeaders/.test(app) && /X-Admin-Token/.test(app)],
  ["运营令牌验证交互", /id="verifyAdminBtn"/.test(html) && /id="adminStatus"/.test(html) && /function verifyAdminAccess/.test(app) && /管理令牌可用/.test(app)],
  ...
  ["运营反馈工单台", /id="feedbackQueue"/.test(html) && /id="refreshFeedbackBtn"/.test(html) && /function renderFeedbackQueue/.test(app) && /function updateFeedbackStatus/.test(app)],
  ["运营审计事件台", /id="auditTrail"/.test(html) && /id="refreshAuditBtn"/.test(html) && /function renderAuditTrail/.test(app) && /function loadAuditTrail/.test(app)],
  ...
  ["AI API 配置区", /id="apiEndpoint"|id="apiKey"|id="modelName"/],
  ...
  ["AI 初稿输出区", /id="aiDraft"/.test(html) && /生成当前姓氏 AI 初稿/.test(html)],
  ["审核队列渲染", /id="reviewQueue"/.test(html) && /function renderReviewQueue/.test(app)],
  ["资料沉淀指标", /id="repositoryStats"/.test(html) && /function renderRepositoryStats/.test(app)],
  ["姓氏档案编辑器", /id="profileEditor"/.test(html) && /id="saveProfileBtn"/.test(html) && /id="profileEditStatus"/.test(html) && /function syncProfileEditor/.test(app) && /function saveProfileEdits/.test(app)],
  ...
  ["资料新增入库", /id="sourceTitle"/.test(html) && /id="sourceContent"/.test(html) && /function addCorpusSource/.test(app)],
  ...
  ["资料导出导入", /id="exportDataBtn"/.test(html) && /id="importDataText"/.test(html) && /function exportWorkspace/.test(app) && /function importWorkspace/.test(app)],
