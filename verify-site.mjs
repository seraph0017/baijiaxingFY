...
const checks = [
  ...
  ["UI 结构验收", /UI 结构检查通过/.test(uiVerify) && /70\/70/.test(uiVerify) && /无重复 ID/.test(uiVerify) && /首页可信证据可见/.test(uiVerify) && /页面标题样式隔离/.test(uiVerify) && /登录页独立后台入口/.test(uiVerify) && /后台工作台指标区/.test(uiVerify) && /后台主线简化/.test(uiVerify) && /后台姓氏一键操作/.test(uiVerify) && /后台不渲染前台专属 DOM/.test(uiVerify) && /前台后台页面分区/.test(uiVerify) && /公开 API 初始化支持沉淀型姓氏库/.test(uiVerify) && /复姓输入支持/.test(uiVerify) && /前台拼音查询支持/.test(uiVerify) && /当前姓氏复用拼音解析/.test(uiVerify) && /未命中拼音不污染姓氏库/.test(uiVerify) && /批量补充过滤拉丁脏输入/.test(uiVerify) && /姓氏双字后缀归一化/.test(uiVerify) && /工作区清空失败提示/.test(uiVerify) && /工作区读取异常提示/.test(uiVerify) && /运营令牌验证/.test(uiVerify) && /种子资料结构完整/.test(uiVerify) && /前端迁徙坐标夹紧/.test(uiVerify) && /前端迁徙空坐标回退/.test(uiVerify) && /迁徙节点不依赖内联样式/.test(uiVerify) && /AI Harness 服务端代理优先/.test(uiVerify) && /AI 初稿字段充实约束/.test(uiVerify) && /AI 初稿自动回填档案字段/.test(uiVerify) && /AI 初稿自动回填结构化分支/.test(uiVerify) && /AI 初稿全字段落库/.test(uiVerify) && /新姓氏默认字段不裸写待补充/.test(uiVerify) && /工作区导入覆盖旧姓氏/.test(uiVerify) && /工作区导入清空缺省队列/.test(uiVerify) && /工作区后端保存串行化/.test(uiVerify) && /工作区导入必须包含姓氏档案/.test(uiVerify) && /工作区导入校验姓氏条目/.test(uiVerify) && /工作区导入校验资料审核条目/.test(uiVerify) && /工作区导入拒绝拉丁混合姓氏/.test(uiVerify) && /工作区快照恢复过滤脏数据/.test(uiVerify) && /工作区导入不回灌旧姓氏/.test(uiVerify) && /工作区导入半成品档案可渲染/.test(uiVerify) && /热门姓氏宫格支持复姓/.test(uiVerify) && /详情姓氏水印绑定身份卡/.test(uiVerify) && /详情身份卡可信信息胶囊/.test(uiVerify) && /详情状态胶囊区分沉淀阶段/.test(uiVerify) && /审核队列稳定 ID/.test(uiVerify) && /批量待收录姓氏/.test(uiVerify) && /审计请求 ID 可见/.test(uiVerify) && /AI Harness 页面文案同步/.test(uiVerify)],
