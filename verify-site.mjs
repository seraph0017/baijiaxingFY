...
const d3Url = new URL("./assets/vendor/d3/d3.min.js", import.meta.url);
const d3 = existsSync(d3Url) ? readFileSync(d3Url, "utf8") : "";
...
const checks = [
  ...
  ["首页后台登录页分离", existsSync(new URL("./admin.html", import.meta.url)) && existsSync(new URL("./login.html", import.meta.url)) && /\/admin/.test(html) && !/\/admin\.html/.test(html) && !/id="opsView"/.test(html)],
  ["前端资源工程化拆分", /<link rel="stylesheet" href="assets\/styles\.css">/.test(html) && /<script src="assets\/vendor\/d3\/d3\.min\.js" defer><\/script>/.test(html) && /<script src="assets\/app\.js" defer><\/script>/.test(html) && !/<style>/.test(html) && !/<script>\s*const surnames/.test(html) && /geoPath/.test(d3) && /d3\.geoPath/.test(app)],
  ...
  ["公开 AI 临时预览", /\/api\/public-ai-draft/.test(server) && /function normalizePublicAiDraftPayload/.test(server) && /function callPublicAiDraft/.test(server) && /public\.ai\.preview/.test(server) && /transient: true/.test(server) && /async function renderPublicSurnameSearch/.test(app) && /function createTransientSurnameFromDraft/.test(app) && /AI 临时初稿/.test(app)],
  ...
  ["服务端超时控制", /const AI_TIMEOUT_MS = readPositiveIntegerEnv\("AI_TIMEOUT_MS", 120_000\)/.test(server) && /const REQUEST_TIMEOUT_MS = readPositiveIntegerEnv\("REQUEST_TIMEOUT_MS", 130_000\)/.test(server) && /AbortSignal\.timeout/.test(server) && /server\.requestTimeout/.test(server)],
  ...
  ["UI 结构验收", /UI 结构检查通过/.test(uiVerify) && /75\/75/.test(uiVerify) && /无重复 ID/.test(uiVerify) && /首页可信证据可见/.test(uiVerify) && /前台 AI 生成等待态完整/.test(uiVerify) && /顶部导航轻量统一/.test(uiVerify) && /页面标题样式隔离/.test(uiVerify) && /登录页独立后台入口/.test(uiVerify) && /后台工作台指标区/.test(uiVerify) && /后台主线简化/.test(uiVerify) && /后台姓氏一键操作/.test(uiVerify) && /后台不渲染前台专属 DOM/.test(uiVerify) && /前台后台页面分区/.test(uiVerify) && /公开 API 初始化支持沉淀型姓氏库/.test(uiVerify) && /前台 AI 临时预览不持久化/.test(uiVerify) && /复姓输入支持/.test(uiVerify) && /前台拼音查询支持/.test(uiVerify) && /当前姓氏复用拼音解析/.test(uiVerify) && /未命中拼音不污染姓氏库/.test(uiVerify) && /批量补充过滤拉丁脏输入/.test(uiVerify) && /姓氏双字后缀归一化/.test(uiVerify) && /工作区清空失败提示/.test(uiVerify) && /工作区读取异常提示/.test(uiVerify) && /运营令牌验证/.test(uiVerify) && /种子资料结构完整/.test(uiVerify) && /前端迁徙坐标夹紧/.test(uiVerify) && /前端迁徙空坐标回退/.test(uiVerify) && /迁徙节点不依赖内联样式/.test(uiVerify) && /迁徙地图区域高亮插件/.test(uiVerify) && /AI Harness 服务端代理优先/.test(uiVerify) && /AI 初稿字段充实约束/.test(uiVerify) && /AI 初稿自动回填档案字段/.test(uiVerify) && /AI 初稿自动回填结构化分支/.test(uiVerify) && /AI 初稿兼容编号标题/.test(uiVerify) && /AI 初稿全字段落库/.test(uiVerify) && /新姓氏默认字段不裸写待补充/.test(uiVerify) && /工作区导入覆盖旧姓氏/.test(uiVerify) && /工作区导入清空缺省队列/.test(uiVerify) && /工作区后端保存串行化/.test(uiVerify) && /工作区导入必须包含姓氏档案/.test(uiVerify) && /工作区导入校验姓氏条目/.test(uiVerify) && /工作区导入校验资料审核条目/.test(uiVerify) && /工作区导入拒绝拉丁混合姓氏/.test(uiVerify) && /工作区快照恢复过滤脏数据/.test(uiVerify) && /工作区导入不回灌旧姓氏/.test(uiVerify) && /工作区导入半成品档案可渲染/.test(uiVerify) && /热门姓氏宫格支持复姓/.test(uiVerify) && /详情姓氏水印绑定身份卡/.test(uiVerify) && /详情身份卡可信信息胶囊/.test(uiVerify) && /详情状态胶囊区分沉淀阶段/.test(uiVerify) && /审核队列稳定 ID/.test(uiVerify) && /批量待收录姓氏/.test(uiVerify) && /审计请求 ID 可见/.test(uiVerify) && /AI Harness 页面文案同步/.test(uiVerify)],
  ...
  ["前台后台页面分区", /id="publicView"/.test(html) && /\/admin/.test(html) && /id="adminApp"/.test(adminHtml) && /\/login/.test(app) && !/\/login\.html/.test(app) && /function requireAdminSession/.test(app)],
  ...
  ["Robots 与 Sitemap", /User-agent: \*/.test(robots) && /Allow: \//.test(robots) && /Sitemap:/.test(robots) && /<urlset/.test(sitemap) && /<loc>__SITE_ORIGIN__\/<\/loc>/.test(sitemap) && !/index\.html/.test(sitemap)],
  ["Web App Manifest", parsedManifest?.name === "百家姓溯源录" && parsedManifest?.short_name === "溯源录" && parsedManifest?.start_url === "/" && parsedManifest?.display === "standalone" && parsedManifest?.theme_color === "#C81623" && parsedManifest?.background_color === "#F9F6EF"],
  ...
  ["AI 初稿输出区", /id="aiDraft"/.test(adminHtml) && /生成 \/ 更新 AI 初稿/.test(adminHtml)],
  ...
  ["审核发布退回动作", /data-action="approve"/.test(app) && /data-action="reject"/.test(app) && /data-review-id/.test(app) && /function updateReviewStatus/.test(app) && /entry\.id === id/.test(app) && /发布到前台/.test(app) && /退回补资料/.test(app) && !/data-title=/.test(app)],
  ...
  ["迁徙路线可视化", /class="migration-map"/.test(app) && /function renderMigrationMap/.test(app) && /function clampMapCoordinate/.test(app) && /data-route-node/.test(app) && /function renderInteractiveMigrationMap/.test(app) && /function resolveMigrationRegions/.test(app) && /migration-region\.active/.test(styles) && /assets\/vendor\/d3\/d3\.min\.js/.test(html)],
  ["现代数字档案馆视觉", /class="app-shell"/.test(html) && /class="hero-panel"/.test(html) && /class="ops-panel"/.test(html) && /section-band/.test(html) && /id="profileMeta"/.test(html) && /AI 可信资料库/.test(html) && /AI 可信资料网络/.test(styles) && /多源并列/.test(html) && /出处标注/.test(html) && /人工审核/.test(html) && /文史编辑工作台/.test(loginHtml) && /admin-overview/.test(adminHtml) && /class="back-home"/.test(html) && /\.hot-list\s*\{[^}]*display:\s*flex/.test(styles) && /\.hot-list\s*\{[^}]*flex-wrap:\s*wrap/.test(styles) && /\.chip\[data-surname-length="2"\]\s*\{[^}]*min-width:\s*96px/.test(styles) && /\.profile-head:after/.test(styles) && /content: attr\(data-surname\)/.test(styles) && /\.profile-meta/.test(styles) && /\.meta-pill/.test(styles) && /function resolveProfileReviewStatus/.test(app) && /来源待核/.test(app) && /\.app-shell/.test(styles) && /\.ops-panel/.test(styles)],
