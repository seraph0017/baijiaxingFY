...
- 如果徐姓不存在，创建“徐姓待收录档案”。
- 把当前后台工作对象切换为徐姓。
- 自动进入 AI Harness 生成流程。
- 如果已有徐姓 Markdown 资料，会带资料生成；如果没有资料，也会先生成基础初稿。
- 生成后加入审核队列。

也可以先点击“切换 / 创建”，确认当前姓氏变成徐，再点击“生成 AI 初稿”。
...
1. 登录 `/admin`。
2. 在“当前姓氏”输入 `徐`，点击“切换 / 创建”。
3. 在“新增资料沉淀”里填写资料标题，例如：
...
保存后点击“生成当前姓氏 AI 初稿”测试。如果配置正确，会显示“已通过服务端 AI 代理生成...”。如果失败，会显示错误摘要，并回退到离线初稿。
